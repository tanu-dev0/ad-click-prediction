...
The dataset (`advertising.csv`) is included at `data/advertising.csv`. Launch Jupyter and run `notebook/ad_click_model.ipynb` — it reads the dataset via a relative path (`../data/advertising.csv`), so no path changes are needed.
